...
def contar_vogais(texto):
    ...
    vogais = "aeiou"  
    contador = 0
    for letra in texto: deveria incluir "AEIOU" ou usar .lower()
        if letra in vogais:
            contador += 1
    return contador
...
def censurar_palavra(texto, palavra_proibida):
    ...
    asteriscos = "*" * len(argumentos invertidos)
    return texto.replace(asteriscos, palavra_proibida) 
...
def estatisticas_notas(notas):
    ...
    return {
        ...
        "maior": max(notas),    deveria ser max(notas)
        "menor": min(notas),    deveria ser min(notas)
    }
...
def gerar_boletim(nome, notas_bimestres):
    ...
    if media >= 6:
        ...
        nota_necessaria = (6*2) - media ou (12 - media) / 1) + media  
...
def ranking_turma(alunos):
    ...
    """
    return sorted(alunos, key=lambda a: a["nota"]
...
def relatorio_final(turma):
    ...
    percentual = len(aprovados) / len(reprovados) /100  
